    ...
    fun retrieveFile(process: (File) -> Unit) {
        driveResourceClient?.let {drive ->
            ...
            rootFolderTask.continueWithTask {task ->
                ...
                val meta = task.result
                val exported = meta.get(0).driveId.asDriveFile()
                drive.openFile(exported, DriveFile.MODE_READ_ONLY)
            }.continueWithTask { task ->
                ...
                contents.inputStream.use {
                    ...
                    zipAll.outputStream().use {zipOut ->
                        it.copyTo(zipOut)
                    }
                    process(zipAll)
                }
                drive.discardContents(contents)
